...
def resample(data_dir):
    ...
    in_path = os.path.join(start_path, "raw")
    out_path = os.path.join(start_path, "wavs")
    subprocess.run(
...
if __name__ == "__main__":
    with gr.Blocks() as app:
        ...
        generate_config_btn.click(
            generate_config, inputs=[data_path, batch_size], outputs=[info]
        )
        resample_btn.click(resample, inputs=[data_path], outputs=[info])
        preprocess_text_btn.click(preprocess_text, inputs=[data_path], outputs=[info])
        bert_gen_btn.click(bert_gen, inputs=[data_path], outputs=[info])
