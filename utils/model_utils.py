...
def load_checkpoint(checkpoint_path, model, optimizer=None, skip_optimizer=False):
    ...
    if optimizer and not skip_optimizer:
        if checkpoint_dict.get("optimizer"):
            optimizer.load_state_dict(checkpoint_dict["optimizer"])
        else:
            print("Warning: No optimizer state found in checkpoint.")
        # Disable this line if Infer and resume checkpoint,then enable the line upper
        # elif optimizer is None and not skip_optimizer:
    elif optimizer and checkpoint_dict.get("optimizer"):
        new_opt_dict = optimizer.state_dict()
        new_opt_dict_params = new_opt_dict["param_groups"][0]["params"]
        new_opt_dict["param_groups"] = checkpoint_dict["optimizer"]["param_groups"]
        new_opt_dict["param_groups"][0]["params"] = new_opt_dict_params
        optimizer.load_state_dict(new_opt_dict)
    elif not optimizer and not skip_optimizer:
        raise ValueError("Optimizer must be provided when not skipping optimizer loading.")
...
def get_logger(model_dir, filename="train.log"):
    ...
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
    file_handel = logging.FileHandler(os.path.join(model_dir, filename))
