...
class GenerateRequest(BaseModel):
    ...
    speech_rate: confloat(ge=0.0, le=2.0) = 1.0  # speech_rate 必须是 0.0 到 2.0 之间的浮点数
    cut_by_sent: bool = False
    audio_format: str = ".wav"  # 默认值 ".wav"
    save_path: Optional[str] = '/mnt/digital_service/audio'  # 音频路径或目录
...
class GenerateResponse(BaseModel):
    time: Union[int, str] = Field(default_factory=lambda: int(time.time() * 100))  # 动态生成时间戳
    staus: int
    id: Union[int, str] = 'jt' 
    sno: Optional[Union[int, str]] = None
    messages: str
    save_path: Optional[str] = None  # 音频文件路径
...
async def post_generate_audio(request: GenerateRequest):
    try:
        ...
        # torch_gc()
        code = 3 if status != 'Failed' else 4
...
async def get_generate_audio(
        ...
        id: Union[int, str] = 'jt',
        content: str = Query(..., min_length=2, description="The text to synthesize"),  # 必需参数，没有默认值
        stream: Optional[bool] = None,
        speaker: Literal['jt', 'huang', 'li'] = 'jt',
        language: Literal['zh', 'en', 'ja', 'mix'] = 'zh',
        speech_rate: confloat(ge=0.0, le=2.0) = 1.0,  # speech_rate 必须是 0.0 到 2.0 之间的浮点数
